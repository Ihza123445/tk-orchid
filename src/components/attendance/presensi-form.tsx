...
export function PresensiForm({ classes }: { classes: Option[] }) {
  ...
  useEffect(() => {
    ...
    setDenied(false)
    let cancelled = false
    loadRosterAction(Number(classId), date).then((res) => {
      if (cancelled) return
      if (!res.allowed) {
        setDenied(true)
        setRows([])
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-3">
        ...
        <div className="space-y-2">
          ...
          <Input id="tanggal" type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-44" />
        </div>
      ...
      {loading ? (
        ...
        <form action={formAction}>
          ...
          <input type="hidden" name="payload" value={JSON.stringify({
            ...
            date,
            entries: rows.map((r) => ({ studentId: r.studentId, status: r.status ?? 'ABSENT', note: r.note ?? undefined })),
          })} />
          ...
          <div className="sticky bottom-0 mt-4 flex justify-end border-t bg-[var(--background)] py-3">
            <Button type="submit" disabled={pending}>Simpan Presensi</Button>
          </div>
        </form>
      ) : !denied ? (
